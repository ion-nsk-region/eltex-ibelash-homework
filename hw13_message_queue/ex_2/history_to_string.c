...
int history_to_string(struct chat_msg *history, int *start_msg_id, int last_msg_id, char *buffer, size_t buffer_size) {
  ...
  if (0 == err) {
    ...
    while (msg != history + last_msg_id && 0 == err) {
      ...
      if (bytes_available > (pid_str_length + 1 + cmd_str_length + 1 + content_length + 1)) {
        ...
        *buf_ptr = 0x1E;  // Record separator = разделитель записей
        ++buf_ptr;
        bytes_available =
            bytes_available - (pid_str_length + 1) - (cmd_str_length + 1) - (content_length + 1);
      } else {
        fprintf(stderr,
                "Предупреждение: недостаточно места в буфере для всей истории.\nbytes_avaialble %ld, start %d, current %ld, last %d\n", bytes_available, *start_msg_id, msg - history, last_msg_id);
        err = ERANGE;
        *start_msg_id = msg - history; // возвращаем номер сообщения, на котором мы остановились.
      }
