...
int main(void) {
  ...
  if (0 == err && -1 < client_mq_id) {
    ...
    for (int i = 0; i < 10; ++i) {
      sprintf(some_msg.content, "Это очень длинное сообщение номер %d. Сейчас мы попытаемся забить буфер так, чтобы серверу приходилось отправлять по два сообщения из истории.", i);
