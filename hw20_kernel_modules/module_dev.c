...
static ssize_t read_from_dev(struct file *fd, char __user *buff, size_t size,
                             loff_t *off) {
  size_t bytes_read,
      test_string_length = strnlen(test_string, MAX_STR_SIZE - 1);

  if (*off >= test_string_length)  // данные уже прочитали
    return 0;

  read_lock(&lock);
  bytes_read =
      simple_read_from_buffer(buff, size, off, test_string, test_string_length);
  read_unlock(&lock);

  pr_debug(
      "Прочитано байт %ld, в буфер %p размером %lu из буфера %p размером %d, "
      "текущая позиция в буфере %lld\nСтрока %s.\n",
      bytes_read, buff, size, test_string, MAX_STR_SIZE, *off, test_string);

  return bytes_read;
}

static ssize_t write_to_dev(struct file *fd, const char __user *buff,
  ...
  write_lock(&lock);
  bytes_written =
      simple_write_to_buffer(test_string, MAX_STR_SIZE, off, buff, size);
  if ('\n' == *(test_string + size - 1)) {
    *(test_string + size - 1) = '\0';  // заменяем перенос строки на конец
  } else {
    *(test_string + size) = '\0';  // иначе, дописываем конец строки
  }
  write_unlock(&lock);

  pr_debug(
      "Записано байт %ld, в буфер %p размером %d из буфера %p размером %lu, "
      "текущая позиция в буфере %lld\nСТрока %s.\n",
      bytes_written, test_string, MAX_STR_SIZE, buff, size, *off, test_string);
...
int init_module(void) {
  ...
  // идея с условной компиляцией взята отсюда:
  ...
  device_ptr = device_create(device_class, NULL, MKDEV(major_number, 0), NULL,
                             DEVICE_NAME);
