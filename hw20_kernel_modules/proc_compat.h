...
#include <linux/version.h>
    ...
    // макро HAVE_PROC_OPS может быть определён и на старых ядрах при 
    // бэкпорте функциональности. Поэтому используем его, а не версию
    // ядря напрямую.
#endif

#ifdef HAVE_PROC_OPS
    // если где-то потребуется тип (proc_ops), то используем следующее макро
...
#   define compat_proc_create(name, mode, parent, ops) \
    ...
    // если где-то потребуется тип структуры, то используем следующее макро
