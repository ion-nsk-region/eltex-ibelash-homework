...
int get_plugin_symbols(const char *plugin_path, void **handle, char **func_name,
                       char **menu_name, unsigned int **menu_pos) {
  char *error;
  ...
  *menu_pos = (unsigned int *)dlsym(*handle, "menu_pos");
  ...
  if (FUNC_MAX < **menu_pos || 0 == **menu_pos) {
    fprintf(stderr,
            "Ошибка: Плагин %s декларирует позицию меню %u\n"
